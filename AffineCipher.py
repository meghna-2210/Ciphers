...
def Affine_Encryption(plainText, a, b):
    ...
    for c in plainText:
        ...
        encrypt_text += chr(value + (a * (asci - value) + b) % 26)
    return encrypt_text


#DECRYPTION
def Affine_Decryption(cypherText, a, b):
    decrypt_text = ""
    inva = inv(a, 26)
    for c in cypherText:
        asci = ord(c)
        value = 97 if asci >= 97 else 65
        decrypt_text += chr(value + inva * (asci - value - b) % 26)
    return decrypt_text
...
if __name__ == "__main__":
    print("AFFINE CIPHER TECHNIQUE")
    plainText = input("Plain Text: ")
    print("Enter keys: ")
    a = int(input())
    b = int(input())
    E = Affine_Encryption(plainText, a, b)
    D = Affine_Decryption(E, a, b)
    print("Original text: {}\nencrypt_text text: {}\ndecrypt_text text: {}".
